...
class M3_Multi_BCE(nn.Module):
    def __init__(self, x_dims, c_dim, z_dims=[16, 8, 4], h_dims=[128, 64]):
        ...
        self.dec1 = nn.Sequential(nn.Linear(z1_dim + c_dim, h1), nn.ReLU(), nn.Linear(h2, x1_dim))
        self.dec2 = nn.Sequential(nn.Linear(z1_dim + z2_dim, h1), nn.ReLU(), nn.Linear(h2, x2_dim))
        self.dec3 = nn.Sequential(nn.Linear(z2_dim + z3_dim, h1), nn.ReLU(), nn.Linear(h2, x3_dim))
        # h_dims 연동을 위해 레이어 조정
